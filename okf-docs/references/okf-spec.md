...
Source: `GoogleCloudPlatform/knowledge-catalog`, `okf/SPEC.md` (fetched 2026-07-07).
This is the full spec text, kept here for reference — `SKILL.md` has the
condensed operating summary.
...
Index files contain no frontmatter (except the bundle-root exception in
§11). The body uses one or more sections, each grouping concepts under a
heading:
