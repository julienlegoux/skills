...
The authoritative copies live in `_shared/` in the dev repo; the copy in each skill's
`references/` is synced at install time. Never edit a synced copy — change the shared
file and re-run the sync.
